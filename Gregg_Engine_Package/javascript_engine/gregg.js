/**
 * ============================================================================
 * GREGG CHARACTER DEFINITIONS - gregg.js
 * ============================================================================
 *
 * PURPOSE:
 * Defines all Gregg shorthand strokes with context-aware rendering logic.
 * Extends Char base class with Gregg-specific path selection rules.
 *
 * SIZE: ~34 KB (1,013 lines)
 *
 * ARCHITECTURE:
 * For each stroke (e.g., K), defines up to 3 variants:
 * 1. GreggK     - Base/standalone (used for single-stroke words)
 * 2. GreggKHead - Entry connector (from previous stroke)
 * 3. GreggKTail - Main stroke body (leads to next stroke)
 *
 * HEAD/TAIL SYSTEM:
 * - Single stroke word: Head + Tail (complete stroke)
 * - First stroke: Tail only (main body, no incoming connector)
 * - Middle strokes: Head + Tail (connector + body)
 * - Last stroke: Head + Tail (connector + body with ending)
 *
 * CONTEXT-AWARE LOGIC:
 * Each stroke's setPaths() method checks neighbors and selects path variant:
 *
 * Example (GreggKTail):
 *   if (this.getNextName() == "GreggI") {
 *     this.setPathsFromObject(PATHS.K.default_tail);  // K blends with I
 *     return;
 *   }
 ...
 * GreggChar.dict = {
 *   "K": [GreggKHead, GreggKTail],  // Array = has head/tail variants
 *   "A": GreggA,                     // Single = no variants (vowel)
 *   "'": GreggH,                     // Special char mapping (aspiration)
 *   "CH": GreggCh,                   // Multi-char token
 * }
 *
 * STROKE LIST:
 * Consonants: K, G, R, L, N, M, T, D, P, B, F, V, CH, J, S, SL, TH, SH
 * Vowels: A, E, I, O, U (circles)
 * Diphthongs: AE, EU, AU, OE, A&E, A&'
 * Blends: NG, NK, LD, TN, DN, TM, DM, NT, MT, ND, MD, DF, DV, TV, SS, XS, MN, MM, DD, TD, DT
 * Special: ' (H aspiration), (, ) (hooks)
 *
 * MODEL NOTATION:
 * ER7 = East Right curve, 7mm | EL14 = East Left curve, 14mm
 * SW = Southwest | SWL = Southwest Left | SWR = Southwest Right
 * SE = Southeast | NE = Northeast | C4 = Circle, size 4 | P = Point
 *
 * SOURCE:
 * Adapted from Grascii editor (MIT License)
 * https://github.com/grascii/grascii-editor
 * Based on Gregg Pre-Anniversary Edition (1916)
 *
 * LICENSE: MIT
 * Gregg shorthand system is in public domain
 *
 * ============================================================================
 */

/**
 * GreggChar - Base class for all Gregg strokes
 * Extends Char with Gregg-specific functionality
 */
...
/**
 * GreggChar.dict - Maps Grascii tokens to constructor(s)
 * Structure: { "TOKEN": Constructor } or { "TOKEN": [HeadConstructor, TailConstructor] }
 */
GreggChar.dict = {};
...
GreggKHead = function() { GreggChar.call(this, "GreggK", "k", "ER7", "ER", "ER", "black", false, p(0.0, 0.7)); };
GreggKHead.prototype = Object.create(GreggChar.prototype);

GreggKHead.prototype.setPaths = function() {
  this.setPathsFromObject(PATHS.K.default_head)
};

GreggKTail = function() { GreggChar.call(this, "GreggK", "k", "ER7", "ER", "ER", "black", false, p(0.0, 0.7)); };
GreggKTail.prototype = Object.create(GreggChar.prototype);

GreggKTail.prototype.setPaths = function() {
  if (this.getNextName() == "GreggI") {
    // K blends with I - use minimal tail connector
    this.setPathsFromObject(PATHS.K.default_tail);
    return;
  }
  if (this.getNextName() == "GreggU") {
    if (this.next.getNextHeadType() == "EL") {
      // K blends with U before L - use minimal tail
      this.setPathsFromObject(PATHS.K.default_tail);
      return;
    }
  }
...
GreggGHead = function() { GreggChar.call(this, "GreggG", "g", "ER14", "ER", "ER", "black", false, p(0.0, 1.3)); };
GreggGHead.prototype = Object.create(GreggChar.prototype);

GreggGHead.prototype.setPaths = function() {
  this.setPathsFromObject(PATHS.G.default_head);
};

GreggGTail = function() { GreggChar.call(this, "GreggG", "g", "ER14", "ER", "ER", "black", false, p(0.0, 1.3)); };
GreggGTail.prototype = Object.create(GreggChar.prototype);

GreggGTail.prototype.setPaths = function() {
  if (this.getNextName() == "GreggI") {
    // G blends with I - use minimal tail connector
    this.setPathsFromObject(PATHS.G.default_tail);
    return;
  }
  if (this.getNextName() == "GreggU") {
    if (this.next.getNextHeadType() == "EL") {
      // G blends with U before L - use minimal tail
      this.setPathsFromObject(PATHS.G.default_tail);
      return;
    }
  }
...
GreggRHead.prototype.setPaths = function() {
  ...
  if (this.getPrevName() == "GreggA") {
    if (this.prev.getPrevTailType() == "SE") {
      return;
    }
  }
...
GreggPTail.prototype.setPaths = function() {
  if (this.getNextHeadType() == "C") {
    if (this.getNextName() == "GreggI") {
      this.setPathsFromObject(PATHS.P.default_tail);  // Always set path
      return;
    }
    ...
    this.setPathsFromObject(PATHS.P.default_tail);  // Fallback
  } else if (this.getNextHeadType() != "EL") {
    this.setPathsFromObject(PATHS.P.default_tail);
    return;
  } else {
    this.setPathsFromObject(PATHS.P.default_tail);  // Fallback for EL
  }
};
...
GreggBTail.prototype.setPaths = function() {
  if (this.getNextHeadType() == "C") {
    if (this.getNextName() == "GreggI") {
      this.setPathsFromObject(PATHS.B.default_tail);  // Always set a path
      return;
    }
    if (!["EL", "ER", "E", "SWR", "NER", "SE"].includes(this.next.getNextHeadType())) {
    ...
    // Fallback: set default path if no other condition matched
    this.setPathsFromObject(PATHS.B.default_tail);
  } else if (this.getNextHeadType() != "EL") {
    this.setPathsFromObject(PATHS.B.default_tail);
    return;
  } else {
    // Fallback for EL case
    this.setPathsFromObject(PATHS.B.default_tail);
  }
};
...
GreggFTail = function() { GreggChar.call(this, "GreggF", "f", "SWR7", "SWR", "SWR", "black", false, p(4.0, -2.9)); };
...
GreggSHead.prototype.setPaths = function() {
  ...
  } else if (["EL", "E", "NE"].includes(this.getPrevTailType())) {
    return;
  }
...
GreggSLTail.prototype.setPaths = function() {
  if (this.getNextHeadType() == "C") {
    if (this.getNextName() == "GreggI") {
      // SL blends with I - use minimal tail connector
      this.setPathsFromObject(PATHS.SL.default_tail);
      return;
    }
    if (["EL", "ER", "E", "SWR"].includes(this.next.getNextHeadType())) {
      // SL blends with vowels before certain consonants - use minimal tail
      this.setPathsFromObject(PATHS.SL.default_tail);
      return;
    }
  }
...
GreggNg.prototype.setPaths = function() {
  this.setPathsFromObject(PATHS.NG.default);
};
...
GreggNk.prototype.setPaths = function() {
  this.setPathsFromObject(PATHS.NK.default);
};
...
// Left hook - small leftward curve modifier (used after vowels for OO sound)
GreggLparen = function() { GreggChar.call(this, "GreggLparen", "(", "C", "C", "C", "black", false, p(0, 0)); };
GreggLparen.prototype = Object.create(GreggChar.prototype);
GreggChar.dict["("] = GreggLparen;

GreggLparen.prototype.setPaths = function() {
  // Small leftward hook - modifies previous vowel
  this.dp = p(1, 1);
  this.paths = ["m 0 0 c -1 0.5, -1 1.5, 0 2"];
};

// Right hook - small rightward curve modifier
GreggRparen = function() { GreggChar.call(this, "GreggRparen", ")", "C", "C", "C", "black", false, p(0, 0)); };
GreggRparen.prototype = Object.create(GreggChar.prototype);
GreggChar.dict[")"] = GreggRparen;

GreggRparen.prototype.setPaths = function() {
  // Small rightward hook - modifies previous vowel
  this.dp = p(1, 1);
  this.paths = ["m 0 0 c 1 0.5, 1 1.5, 0 2"];
};
...
// ^ (disjoiner) is handled in Char.dict, not GreggChar.dict

// aliases for combo strokes and unmodeled strokes
GreggChar.dict["Z"] = GreggS;  // Z uses S stroke
...
GreggChar.dict["MM"] = [GreggM, GreggM];  // Fixed: was [M, N]
GreggChar.dict["DT"] = [GreggD, GreggT];
GreggChar.dict["TD"] = [GreggT, GreggD];  // Fixed: was [D, T]
GreggChar.dict["DD"] = [GreggD, GreggD];  // Fixed: was [D, T]
GreggChar.dict["A&'"] = GreggA;
